...
export default function SearchedSeriesCard({ series, createSerieCb, disabled }: { disabled?: boolean, series: OMDBSearchSeries, createSerieCb: (id: string) => Promise<void> }) {
  ...
  return (
        ...
        >
            <div className="flex flex-col mx-auto relative w-[180px]">
                ...
                <div
                    className={`absolute left-2/4 top-2/4 transition-all ease-in duration-150 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                    style={{ transform: 'translate(-50%, -50%)' }}
                >
                    <button
                        className="bg-primary hover:bg-primary-hover font-bold py-2 px-4 rounded-full text-base shadow-sm"
                        onClick={() => createSerieCb(series.imdbID)}
                        disabled={disabled}
                    >
