...
import { AllMoviesDocument, AllMoviesQuery, AllMoviesQueryVariables, AllMoviesLangGenreQuery, AllMoviesLangGenreDocument, AllMoviesLangGenreQueryVariables, SortOrder } from '../../generated/graphql';
...
export default function Movies({ movies, genres, languages }: { movies: AllMoviesLangGenreQuery['movies'], genres: AllMoviesLangGenreQuery['genres'], languages: AllMoviesLangGenreQuery['languages'] }) {
  ...
  const [cursor, setCursor] = useState<number>(movies?.[movies.length - 1]?.id || 0);
  const [moviesCursor, setMoviesCursor] = useState<AllMoviesLangGenreQuery['movies']>(movies || []);
  const years: number[] = generateYearsArray(1960);
  const [loadMore, setLoadMore] = useState<boolean>(false);

  const loadMoreMovies = async () => {
    setLoadMore(true);
    const { data, error, loading } = await apolloClient.query<AllMoviesQuery, AllMoviesQueryVariables>({
    ...
    <Layout home={false}>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4 md:gap-12">
        <>
          ...
          {moviesCursor?.map(m => <MovieCard movie={m} key={m.id} />)}
        </>
      </section>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const { query } = context;
  const { data, error } = await apolloClient.query<AllMoviesLangGenreQuery, AllMoviesLangGenreQueryVariables>({
