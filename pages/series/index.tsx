...
import { SortOrder, GenresLanguagesQuery, AllSeriesLangGenreQuery, AllSeriesLangGenreQueryVariables, AllSeriesLangGenreDocument, AllSeriesDocument, AllSeriesQuery, AllSeriesQueryVariables } from '../../generated/graphql';
import { useRouter } from 'next/dist/client/router';
import SeriesCard from '../../components/items/SeriesCard';
import { StringListDropdown } from '../../components/utils/dropdown';
...
export default function SeriesPage({ series, genres, languages }: { series: AllSeriesLangGenreQuery['series'], genres: AllSeriesLangGenreQuery['genres'], languages: GenresLanguagesQuery['languages'] }) {

  const router = useRouter();
  const { g, l, r, i } = router.query;
  const [genreQuery, setGenreQuery] = useState<string | string[]>(g as string || []);
  const [languageQuery, setLanguageQuery] = useState<string | string[]>(l as string || []);
  const [yearQuery, setYearQuery] = useState<string | string[]>(r as string || []);
  const [imdbRatingQuery, setImdbRatingQuery] = useState<string | string[]>(i as string || []);
  const [cursor, setCursor] = useState<number>(series?.[series.length - 1]?.id || 0);
  const [seriesCursor, setSeriesCursor] = useState<AllSeriesLangGenreQuery['series']>(series || []);
  const years: number[] = generateYearsArray(1960);
  const [loadMore, setLoadMore] = useState<boolean>(false);

  const loadMoreSeries = async () => {
    setLoadMore(true);
    const { data, error, loading } = await apolloClient.query<AllSeriesQuery, AllSeriesQueryVariables>({
    ...
    <Layout home={false}>
        ...
        <div className="flex flex-row gap-x-6 gap-y-2 flex-wrap">
          {
            ...
            <StringListDropdown
              ...
              defaultValues={
                Array.isArray(genreQuery) ?
                  genreQuery.map(gen => { return { label: gen, value: gen }; })
                  :
                  [{ label: genreQuery, value: genreQuery }]
              }
              items={genres.map(gen => { return { label: gen.name, value: gen.name }; })}
          ...
          }
          {
            ...
            <StringListDropdown
              ...
              defaultValues={
                Array.isArray(languageQuery) ?
                  languageQuery.map(lang => { return { label: lang, value: lang }; })
                  :
                  [{ label: languageQuery, value: languageQuery }]
              }
              items={languages.map(lang => { return { label: lang.name, value: lang.name }; })}
              onChange={(items) => setLanguageQuery(items.map(item => item.value))}
            />
          }
          <StringListDropdown
            label='Release'
            defaultValues={
              Array.isArray(yearQuery) ?
                yearQuery.map(year => { return { label: year.toString(), value: year.toString() }; })
                :
                [{ label: yearQuery, value: yearQuery }]
            }
            items={years.map(year => { return { label: year.toString(), value: year.toString() }; })}
          ...
          <StringListDropdown
            ...
            defaultValues={
              Array.isArray(imdbRatingQuery) ?
                imdbRatingQuery.map(rating => { return { label: rating.toString(), value: rating.toString() }; })
                :
                [{ label: imdbRatingQuery, value: imdbRatingQuery }]
            }
            items={imdbRatingsArray.map(ir => { return { label: `${ir.toString()} ⭐`, value: ir.toString() }; })}
            onChange={(items) => setImdbRatingQuery(items.map(it => it.value))}
          />
        </div>
      </section>
      <section className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4 md:gap-12">
        <>
          ...
          {seriesCursor?.map(s => <SeriesCard series={s} key={s.id} />)}
        </>
      </section>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const { query } = context;
  const { data, error } = await apolloClient.query<AllSeriesLangGenreQuery, AllSeriesLangGenreQueryVariables>({
